...
import {
	AttachmentService,
	MirrorResult,
} from './src/service/attachment-service';
...
async function resolveShell(): Promise<typeof shell> {
	// Optional-chain the runtime guard: under context isolation the renderer's
	// electron.shell is undefined even though its type says otherwise, and a
	// direct property read would throw. A missing shell falls through to
	// @electron/remote below.
...
export default class LinkedAttachmentsPlugin extends Plugin {
	...
	async onload(): Promise<void> {
		...
		this.addCommand({
			...
			checkCallback: (checking: boolean): boolean => {
				const ready =
					this.settings.endpoint.length > 0 &&
					this.settings.bucket.length > 0 &&
					this.credentials.hasCompleteCredentials();
				if (!ready) {
					return false;
				}
				if (!checking) {
					new TrustRehearsalModal(
						...
						this.logger,
						(error) => {
							this.logger.error('Trust rehearsal threw.', {
			...
			checkCallback: (checking: boolean): boolean => {
				...
				if (!checking) {
					new ReconcileModal(this.app, this.attachments, (error) => {
						this.logger.error('Reconcile threw.', {
							error: describeError(error),
						});
					}).open();
				}
		...
		this.addCommand({
			...
			checkCallback: (checking: boolean): boolean => {
				...
				if (
					file === null ||
					this.pointerCopyState(file) === 'checked-out'
				) {
					return false;
				}
				if (!checking) {
		...
		this.registerMarkdownPostProcessor((element) => {
			const links = element.querySelectorAll<HTMLAnchorElement>(
				'a[href^="obsidian://linked-attachments"]',
			);
			links.forEach((link) => {
			...
			this.app.workspace.on('file-menu', (menu, file) => {
				...
				if (file.extension === 'md') {
					...
					if (
						...
						'la_version' in frontmatter
					) {
						menu.addItem((item) => {
							item.setTitle('Restore from storage')
						...
						menu.addItem((item) => {
							item.setTitle('Check storage status')
						...
						menu.addItem((item) => {
							item.setTitle('Copy storage reference')
						...
						if (Platform.isDesktop) {
							...
							menu.addItem((item) => {
								item.setTitle(
									'Reveal local copy in file explorer',
								)
									.setIcon('folder-open')
									.onClick(() => {
						...
						if (Platform.isDesktop) {
							if (
								frontmatter['la_copy_state'] === 'checked-out'
							) {
								menu.addItem((item) => {
									item.setTitle(
										'Check in (save a new version)',
									)
								...
								menu.addItem((item) => {
									item.setTitle('Discard checkout')
										.setIcon('rotate-ccw')
										.onClick(() => {
											void this.runDiscardCheckout(file);
										});
								...
								menu.addItem((item) => {
									item.setTitle('Check out to edit')
										.setIcon('file-edit')
										.onClick(() => {
											void this.runCheckout(file);
										});
				...
				menu.addItem((item) => {
					item.setTitle('Offload to storage')
		...
		this.autoOffload = new AutoOffloadController({
			...
			onError: (error) =>
				this.logger.error('Auto-offload threw.', {
					error: describeError(error),
				}),
		});
	...
	private async runCheckout(pointer: TFile): Promise<void> {
		...
		try {
			...
			if (
				!result.ok &&
				(result.lockState === 'held-by-other' ||
					result.lockState === 'stale')
			) {
				// Advisory only: offer a forced checkout (the lock cannot be enforced).
				const force = await this.confirm(
					'Force a checkout?',
					`${result.error ?? 'This file is checked out elsewhere.'} Force a checkout anyway?`,
					'Force checkout',
				);
				if (!force) {
					new Notice(
						result.error ??
							'This file is checked out on another device.',
					);
					return;
				}
				result = await this.attachments.checkout(pointer, {
					force: true,
				});
			}
			if (result.ok) {
				...
				new Notice(
					`Checked out ${pointer.basename}. Edit it in the app that opened, then check it in to save a new version.`,
				);
				void this.updateDirtyStatus();
			} else {
				...
				new Notice(
					state.isAuth
						? state.message
						: `Could not check out ${pointer.basename}: ${result.error ?? 'unknown error'}.`,
				);
			}
		} catch (error) {
			new Notice(
				`Checkout of ${pointer.basename} failed. See the log for details.`,
			);
			this.logger.error('Checkout threw.', {
				path: pointer.path,
				error: describeError(error),
			});
		}
		...
		try {
			...
			if (result.ok) {
				...
				if (result.kind === 'no-op') {
					new Notice(
						`${pointer.basename} was unchanged; checkout released.`,
					);
				} else if (result.kind === 'conflict') {
					new Notice(
						`Checked in ${pointer.basename} as a new version. The cloud had changed meanwhile, so the previous version was kept as a conflict copy.`,
					);
				} else {
					new Notice(
						`Checked in ${pointer.basename} as a new version.`,
					);
				}
				void this.updateDirtyStatus();
			} else {
				...
				new Notice(
					state.isAuth
						? state.message
						: `Could not check in ${pointer.basename}: ${result.error ?? 'unknown error'}. Your edits are kept.`,
				);
			}
		} catch (error) {
			new Notice(
				`Check-in of ${pointer.basename} failed. Your edits are kept. See the log for details.`,
			);
			this.logger.error('Checkin threw.', {
				path: pointer.path,
				error: describeError(error),
			});
		}
	...
	private async runDiscardCheckout(pointer: TFile): Promise<void> {
		const ok = await this.confirm(
			'Discard checkout?',
			`Discard your checked-out edits to ${pointer.basename}? The working copy is deleted and nothing is uploaded.`,
			'Discard edits',
		);
		if (!ok) {
			return;
		...
		try {
			...
			if (result.ok) {
				...
				new Notice(
					`Could not discard the checkout: ${result.error ?? 'unknown error'}.`,
				);
			}
		} catch (error) {
			new Notice(
				`Discarding the checkout of ${pointer.basename} failed. See the log for details.`,
			);
			this.logger.error('Discard checkout threw.', {
				path: pointer.path,
				error: describeError(error),
			});
		}
	...
	private async updateDirtyStatus(): Promise<void> {
		...
		if (
			...
			this.dirtyStatusEl.removeClass(
				'linked-attachments-dirty-green',
				'linked-attachments-dirty-orange',
				'linked-attachments-dirty-red',
			);
			return;
		}
		try {
			...
			this.dirtyStatusEl.setText('');
			this.logger.warn('Dirty status read failed.', {
				path: file.path,
				error: describeError(error),
			});
		}
	...
	private activePointer(): TFile | null {
		...
		const frontmatter =
			this.app.metadataCache.getFileCache(file)?.frontmatter;
		return frontmatter !== undefined && 'la_version' in frontmatter
			? file
			: null;
	}
	...
	private runBatchOffload(files: TFile[]): void {
		...
		new BatchOffloadModal(this.app, this.attachments, files, (error) => {
			this.logger.error('Batch offload threw.', {
				error: describeError(error),
			});
		}).open();
	}
	...
	runVaultSweep(): void {
		...
		const plan = planVaultSweep(
			candidates,
			normalizeRules(this.settings.offloadRules),
		);
		...
		if (plan.selected.length === 0) {
			new Notice(
				'No files in your vault match the offload rules. Add or adjust rules in settings.',
			);
			return;
		}
	...
	private async runResume(): Promise<void> {
		...
		try {
			...
			if (result.journals === 0) {
				...
				new Notice(
					`Resumed ${result.resumed} file(s) from ${result.journals} session(s)${result.failed > 0 ? `, ${result.failed} still need attention` : ''}.`,
				);
			}
	...
	private async copyPointerReference(pointer: TFile): Promise<void> {
		try {
			const record = decodePointer(
				await this.app.vault.read(pointer),
			).record;
			...
			new Notice(
				'Could not copy the storage reference. See the log for details.',
			);
			this.logger.warn('Copy reference failed.', {
				path: pointer.path,
				error: describeError(error),
			});
		}
	...
	private async showPointerStatus(pointer: TFile): Promise<void> {
		try {
			...
			this.logger.warn('Pointer status read failed.', {
				path: pointer.path,
				error: describeError(error),
			});
		}
	...
	private async runOffload(file: TFile): Promise<void> {
		...
		try {
			...
			new Notice(
				`Could not prepare the offload of ${file.name}. See the log for details.`,
			);
			this.logger.error('Offload preview failed.', {
				path: file.path,
				error: describeError(error),
			});
		}
	...
	private async handleOpenProtocol(
		params: ObsidianProtocolData,
	): Promise<void> {
		...
		this.logger.info('Pointer link invoked.', {
			op: op ?? '(none)',
			hasId: typeof params.id === 'string',
		});
		const id = params.id;
		...
		const backend: BackendType | undefined =
			params.backend === 's3' || params.backend === 'local'
				? params.backend
				: undefined;
		switch (op) {
			case 'reveal':
	...
	private async openRecord(
		...
		if (preferred !== 's3') {
			...
			if (localPath !== null) {
				...
				if (
					hasS3 &&
					(await this.attachments.localCopyIsDataless(record))
				) {
					new Notice(
						'The local copy is online-only (not downloaded yet); opening the S3 copy instead...',
					);
					s3Announced = true;
			...
			if (!s3Announced) {
				...
				if (!hasS3) {
					const hasLocal = record.backends.some(
				...
				new Notice(
					'The local copy is not on disk; opening the S3 copy instead...',
				);
			}
		} else {
		...
		const openDatalessFallback = async (): Promise<boolean> => {
			...
			new Notice(
				'Could not open the S3 copy; opening the local copy instead...',
			);
			return this.openInShell(datalessLocalFallback);
		};
		try {
			const tempPath = await this.attachments.downloadForOpen(
				record,
				's3',
			);
			if (tempPath === null) {
			...
			this.logger.error('Open via download failed.', {
				id: record.id,
				error: describeError(error),
			});
		}
	...
	private async openInShell(absolutePath: string): Promise<boolean> {
		...
		try {
			...
			this.logger.error('shell.openPath threw.', {
				path: absolutePath,
				error: describeError(error),
			});
			return false;
	...
	private async runRefreshPointerBlocks(): Promise<void> {
		try {
			...
			new Notice(
				`Refreshing pointer links failed: ${describeError(error)}`,
			);
		}
	}
	...
	private warnIfAutoOffloadUnconfigured(): void {
		...
		const usesLocal =
			this.settings.storageMode === 'local-only' ||
			this.settings.storageMode === 'local-s3';
		if (usesLocal && this.resolvedLocalRoot().length === 0) {
			new Notice(
				'Auto-offload is on, but this machine has no local folder set. New files will not be offloaded until you set it in settings.',
			);
		}
	}
	...
	private async runIntegrityCheck(): Promise<void> {
		...
		try {
			...
			if (findings.length === 0) {
				new Notice(
					'All pointers healthy: every backend holds its object.',
				);
				return;
			}
			new Notice(
				`${findings.length} pointer(s) have a missing backend. See the activity log for details.`,
			);
	...
	async runAddMirror(target: 'local' | 's3'): Promise<MirrorResult | null> {
		...
		try {
			const result =
				target === 'local'
					? await this.attachments.addLocalMirror()
					: await this.attachments.addS3Mirror();
			new Notice(
				`${label} mirror: ${result.added} added, ${result.skipped} skipped, ${result.failed} failed.`,
			);
			this.logger.info('Add mirror complete.', {
				target,
				added: result.added,
			...
			new Notice(
				`Adding ${label} mirror failed: ${describeError(error)}`,
			);
			this.logger.error('Add mirror failed.', {
				target,
				error: describeError(error),
			});
			return null;
	...
	private async executeOffload(file: TFile): Promise<void> {
		...
		try {
			...
			if (result.ok) {
				...
				this.logger.info('Offload finished.', {
					path: file.path,
					removed: result.removed,
					tier: result.record?.verificationTier,
				});
			} else {
				...
				new Notice(
					state.isAuth
						? state.message
						: offloadOutcomeLine(file.name, result),
				);
				this.logger.warn('Offload did not complete.', {
					path: file.path,
					stage: result.reachedStage,
					error: result.error,
					authFailure: state.isAuth,
				});
			}
		} catch (error) {
			new Notice(
				`Offload of ${file.name} failed. See the log for details.`,
			);
			this.logger.error('Offload threw.', {
				path: file.path,
				error: describeError(error),
			});
		}
		...
		try {
			...
			if (result.ok) {
				...
				new Notice(
					state.isAuth
						? state.message
						: `Restore did not complete: ${result.error ?? 'unknown error'}.`,
				);
				this.logger.warn('Restore did not complete.', {
					pointer: pointer.path,
					error: result.error,
					authFailure: state.isAuth,
				});
			...
			new Notice(
				state.isAuth
					? state.message
					: `Restore from ${pointer.name} failed. See the log for details.`,
			);
			this.logger.error('Restore threw.', {
				pointer: pointer.path,
				error: describeError(error),
				authFailure: state.isAuth,
			});
	...
	async loadSettings(): Promise<void> {
		...
		const raw = (await this.loadData()) as
			| (Partial<LinkedAttachmentsSettings> & {
					autoOffloadAllowlist?: string;
		...
		const migrated = migratedLocalAttachment(
			raw?.localAttachment,
			raw?.localRoot,
			activeMachine(),
		);
		...
		if (migrated !== null) {
			...
			this.settings.localAttachment = {
				machines: (this.settings.localAttachment.machines ?? []).map(
					(entry) => ({ ...entry }),
				),
			};
		}
		...
		const renamedAccess =
			RENAMED_SECRET_IDS[this.settings.accessKeyIdSecretName];
		if (renamedAccess !== undefined) {
			this.settings.accessKeyIdSecretName = renamedAccess;
		}
		const renamedSecret =
			RENAMED_SECRET_IDS[this.settings.secretAccessKeySecretName];
		...
		if (this.settings.secretAccessKeySecretName.length === 0) {
			this.settings.secretAccessKeySecretName =
				DEFAULT_SECRET_KEY_SECRET_ID;
		}
		...
		if (
			raw !== null &&
			raw.offloadRules === undefined &&
			typeof raw.autoOffloadAllowlist === 'string'
		) {
			const threshold =
				typeof raw.autoOffloadSizeThresholdMb === 'number'
					? raw.autoOffloadSizeThresholdMb
					: 5;
			this.settings.offloadRules = rulesFromLegacy(
				raw.autoOffloadAllowlist,
				threshold,
			);
		}
