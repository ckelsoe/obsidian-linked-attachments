...
describe('offload rule policy (per-extension)', () => {
	...
	it('test_always_rule_offloads_regardless_of_size', () => {
		...
		const d = decideByRules(
			candidate({ extension: 'epub', size: 1 }),
			rules,
		);
	...
	it('test_over_size_rule_respects_its_threshold', () => {
		const rules = [
			rule({ extension: 'pdf', mode: 'over-size', thresholdMb: 10 }),
		];
		expect(
			decideByRules(candidate({ extension: 'pdf', size: 10 * MB }), rules)
				.offload,
		).toBe(true);
		const under = decideByRules(
			candidate({ extension: 'pdf', size: 10 * MB - 1 }),
			rules,
		);
	...
	it('test_per_extension_thresholds_are_independent', () => {
		...
		expect(
			decideByRules(candidate({ extension: 'pdf', size: 5 * MB }), rules)
				.offload,
		).toBe(true);
		expect(
			decideByRules(candidate({ extension: 'mp4', size: 5 * MB }), rules)
				.offload,
		).toBe(false);
	});
	...
	it('test_unlisted_type_never_offloads', () => {
		...
		const d = decideByRules(
			candidate({ extension: 'txt', size: 999 * MB }),
			rules,
		);
	...
	it('test_match_case_insensitive', () => {
		const rules = [rule({ extension: 'epub', mode: 'always' })];
		expect(
			decideByRules(candidate({ extension: 'EPUB', size: 1 }), rules)
				.offload,
		).toBe(true);
	});
	...
	it('test_threshold_boundary_inclusive', () => {
		...
		expect(
			decideByRules(candidate({ extension: 'pdf', size: 1 * MB }), rules)
				.offload,
		).toBe(true);
		expect(
			decideByRules(
				candidate({ extension: 'pdf', size: 1 * MB - 1 }),
				rules,
			).offload,
		).toBe(false);
	});
	...
	it('test_disabled_rule_offloads_nothing', () => {
		const off = decideByRules(
			candidate({ extension: 'epub', size: 999 * MB }),
		...
		expect(
			decideByRules(candidate({ extension: 'epub', size: 1 }), [
		...
		fc.assert(
			fc.property(
				...
				(sizeMb, thresholdMb) => {
					...
					const d = decideByRules(
						candidate({ extension: 'bin', size: sizeMb * MB }),
						rules,
					);
