...
async function expectationFor(
	bytes: Uint8Array,
	extra: Partial<LadderExpectation> = {},
): Promise<LadderExpectation> {
	return {
...
const ALL_TIERS: VerificationTier[] = [
	'asserted',
	'existence',
	'md5',
	'content',
...
describe('verify ladder acceptance (la-p2-08)', () => {
	...
	it('test_checksum_rung_content', async () => {
		...
		await backend.put('k', bytes, bytes.length, {
			checksumSha256: await sha256Base64(bytes),
		});
		const result = await verifyByLadder(
			backend,
			'k',
			await expectationFor(bytes),
		);
	...
	it('test_checksum_rung_content_when_head_size_unknown', async () => {
		...
		await inner.put('k', bytes, bytes.length, {
			checksumSha256: await sha256Base64(bytes),
		});
	...
	it('test_md5_rung', async () => {
		...
		await backend.seedObject('k', bytes, {
	...
	it('test_refuse_floor_missing_object', async () => {
		const backend = new MemoryBackend();
		let deleteCalls = 0;
		const decision = await verifyBeforeDelete(
			backend,
			'missing',
			await expectationFor(bytesOf('x')),
			() => {
				deleteCalls++;
	...
	it('test_delete_gate_min_tier', () => {
		...
		expect(canHardDelete('asserted', { allowAssertedDelete: true })).toBe(
			true,
		);
	...
	it('test_never_delete_on_drift', async () => {
		...
		const expectation = await expectationFor(
			bytesOf('these are the right bytes okay'),
	...
	it('prop_gate_is_threshold', () => {
		fc.assert(
			fc.property(
				...
				fc.constantFrom(...ALL_TIERS),
				(tier, min) => {
					expect(canHardDelete(tier, { minimumTier: min })).toBe(
						RANK[tier] >= RANK[min],
					);
				},
			),
	...
	it('prop_asserted_optin_allows_all', () => {
		fc.assert(
			fc.property(fc.constantFrom(...ALL_TIERS), (tier) => {
				expect(canHardDelete(tier, { allowAssertedDelete: true })).toBe(
					true,
				);
			}),
			{ numRuns: 50 },
...
describe('verify ladder failure injection (la-p2-08)', () => {
	...
	it('fault_delete_callback_throw_surfaces', async () => {
		...
		await expect(
			verifyBeforeDelete(backend, 'k', await expectationFor(bytes), () =>
