...
describe('K1 text-extract acceptance (la-p1-06)', () => {
	...
	it('test_extracts_text', async () => {
		const result = await extractText(
			'pdf',
			bytes('ignored'),
			fixedExtractor('Romans 1:17 the righteous shall live by faith'),
		);
	...
	it('test_extractor_failure_explicit', async () => {
		const result = await extractText(
			'pdf',
			bytes('corrupt'),
			throwingExtractor('stream decode error'),
		);
	...
	it('test_extracted_sidecar_contains_text', async () => {
		const result = await extractText(
			'epub',
			bytes('book'),
			fixedExtractor('chapter one had a long body of searchable text'),
		);
		const sidecar = renderTextSidecar(result, 'book.epub');
		expect(sidecar).toContain(
			'chapter one had a long body of searchable text',
		);
	});
});
		...
		await fc.assert(
			fc.asyncProperty(fc.string(), async (output) => {
				...
				if (result.state === 'extracted') {
					...
					expect((result.text ?? '').trim().length).toBeGreaterThan(
						0,
					);
				}
			}),
...
describe('K1 text-extract failure injection (la-p1-06)', () => {
	...
	it('fault_extractor_rejection_contained', async () => {
		await expect(
			extractText('pdf', bytes('x'), throwingExtractor('boom')),
	...
	it('fault_no_sidecar_for_unsupported_or_failed', async () => {
		const unsupported = await extractText(
			'png',
			bytes('img'),
			fixedExtractor('x'),
		);
