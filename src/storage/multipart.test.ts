...
describe('multipart URLs', () => {
	it('buildListUploadsUrl asks for ?uploads on the bucket', () => {
		expect(buildListUploadsUrl(config)).toBe(
			'https://s3.us-east-1.amazonaws.com/b?uploads',
		);
	});
	it('buildAbortUploadUrl targets the key with the upload id', () => {
		expect(buildAbortUploadUrl(config, 'books/a.pdf', 'abc123')).toBe(
			'https://s3.us-east-1.amazonaws.com/b/books/a.pdf?uploadId=abc123',
		);
	});
	it('buildAbortUploadUrl percent-encodes a key with spaces', () => {
		expect(buildAbortUploadUrl(config, 'a b.pdf', 'x')).toBe(
			'https://s3.us-east-1.amazonaws.com/b/a%20b.pdf?uploadId=x',
		);
	});
});
...
describe('parseMultipartUploads', () => {
	it('parses key, uploadId, and initiated per upload', () => {
		...
		expect(uploads[0]).toEqual({
			key: 'a.pdf',
			uploadId: 'id-1',
			initiated: '2026-06-17T00:00:00.000Z',
		});
...
describe('cleanupIncompleteUploads', () => {
	...
	it('an abort failure is counted, the rest still run', async () => {
		const transport: MultipartTransport = {
			list: () => Promise.resolve({ status: 200, text: listXml }),
			abort: (key) =>
				Promise.resolve({ status: key === 'a.pdf' ? 500 : 204 }),
		};
		const result = await cleanupIncompleteUploads(transport);
	...
	it('a clean bucket aborts nothing', async () => {
		const transport: MultipartTransport = {
			list: () =>
				Promise.resolve({
					status: 200,
					text: '<ListMultipartUploadsResult></ListMultipartUploadsResult>',
				}),
			abort: () => Promise.resolve({ status: 204 }),
		};
		expect(await cleanupIncompleteUploads(transport)).toEqual({
			found: 0,
			aborted: 0,
			failed: 0,
		});
