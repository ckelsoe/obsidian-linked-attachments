...
describeLive(
	'incomplete-multipart cleanup live on R2 (la-p4-24, gated)',
	() => {
		...
		afterAll(async () => {
			...
			if (uploadId !== null) {
				await signedSend(
					config,
					'DELETE',
					buildAbortUploadUrl(config, key, uploadId),
				).catch(() => undefined);
			}
		});
		...
		it('initiates then aborts a multipart upload by its exact id (CreateMultipart + AbortMultipart)', async () => {
			...
			const initiated = await signedSend(
				config,
				'POST',
				`${objectUrl(config, key)}?uploads`,
			);
			...
			const aborted = await signedSend(
				config,
				'DELETE',
				buildAbortUploadUrl(config, key, uploadId ?? ''),
			);
		...
		it('ListMultipartUploads returns a parseable listing (the LIST verb signs and parses)', async () => {
			const listed = await signedSend(
				config,
				'GET',
				buildListUploadsUrl(config),
			);
			expect(listed.status).toBe(200);
			// The parser accepts R2's real response shape (array, possibly empty).
			expect(Array.isArray(parseMultipartUploads(listed.text))).toBe(
				true,
			);
		});
		...
		it('cleanupIncompleteUploads runs end to end live and sweeps any orphans', async () => {
			const transport: MultipartTransport = {
				list: () =>
					signedSend(config, 'GET', buildListUploadsUrl(config)),
				abort: async (k, id) => ({
					status: (
						await signedSend(
							config,
							'DELETE',
							buildAbortUploadUrl(config, k, id),
						)
					).status,
				}),
			};
