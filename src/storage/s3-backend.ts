...
export class S3Backend implements StorageBackend {
	...
	async put(
		...
		const response = await this.send(
			'PUT',
			objectUrl(this.deps.config, key),
			{ headers, body: toArrayBuffer(bytes), payloadHashHex },
		);
	...
	async list(prefix = '', opts: ListOptions = {}): Promise<ListPage> {
		...
		return {
			...
			isTruncated,
			cursor: isTruncated ? nextToken : null,
		};
	}
	...
	private async send(
		...
		if (creds === null) {
			throw new BackendError(
				'auth',
				'credentials are not configured (no key in secret storage)',
			);
		}
		const signed = await signRequest({
			...
			url,
			region:
				this.deps.config.region.length > 0
					? this.deps.config.region
					: 'us-east-1',
			service: 's3',
	...
	private error(
		...
		if (authStatus || (code !== null && AUTH_CODES.has(code))) {
			return new BackendError(
				'auth',
				`${context}: authentication/authorization failed (HTTP ${response.status}${code !== null ? ` ${code}` : ''})`,
			);
		}
		return new BackendError(
			'network',
			`${context}: HTTP ${response.status}${code !== null ? ` (${code})` : ''}`,
		);
	}
}
