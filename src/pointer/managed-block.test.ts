...
describe('renderManagedBlock', () => {
	...
	it('emits a callout with op-keyed links per backend', () => {
		...
		expect(block).toContain(
			'> - Local: [open](obsidian://linked-attachments?op=open&backend=local&id=la-1) · [reveal](obsidian://linked-attachments?op=reveal&backend=local&id=la-1)',
		);
		expect(block).toContain(
			'> - S3: [open](obsidian://linked-attachments?op=open&backend=s3&id=la-1) · [copy reference](obsidian://linked-attachments?op=copy&backend=s3&id=la-1)',
		);
	...
	it('falls back to a bare open row inside the callout when no backend matches', () => {
		...
		expect(block).toContain(
			'> - [open](obsidian://linked-attachments?op=open&id=la-3)',
		);
	});
