...
describe('reference scanner acceptance (la-p1-03)', () => {
	...
	it('test_recognizes_explicit_pointer_form', () => {
		expect(referencesAttachment('Cranfield.pdf.md', 'Cranfield.pdf')).toBe(
			true,
		);
	});
	...
	it('test_recognizes_basename_form', () => {
		expect(referencesAttachment('Cranfield.pdf', 'Cranfield.pdf')).toBe(
			true,
		);
	...
	it('test_recanonicalize_basename_to_explicit', () => {
		const out = rewriteEmbedsToPointer(
			'see ![[Cranfield.pdf]] here',
			'Cranfield.pdf',
		);
	...
	it('test_restore_reverses_to_attachment', () => {
		const out = rewriteEmbedsToAttachment(
			'see ![[Cranfield.pdf.md]] here',
			'Cranfield.pdf',
		);
	...
	it('test_alias_and_subpath_preserved', () => {
		const out = rewriteEmbedsToPointer(
			'![[Cranfield.pdf#page=3|Romans commentary]]',
			'Cranfield.pdf',
		);
	...
	it('test_embed_vs_link_distinguished', () => {
		const refs = scanReferences(
			'embed ![[Cranfield.pdf]] and link [[Cranfield.pdf.md]]',
		);
	...
	it('test_unrecognized_shape_blocks_gc', () => {
		const md = analyzeForGc(
			'![diagram](attachments/Cranfield.pdf)',
			'Cranfield.pdf',
		);
		...
		fc.assert(
			fc.property(
				...
				(before, after) => {
					...
					const toPointer = rewriteEmbedsToPointer(
						original,
						'Cranfield.pdf',
					);
					const back = rewriteEmbedsToAttachment(
						toPointer.text,
						'Cranfield.pdf',
					);
...
describe('reference scanner failure injection (la-p1-03)', () => {
	...
	it('fault_embed_in_inline_code_ignored', () => {
		const out = rewriteEmbedsToPointer(
			'`![[Cranfield.pdf]]` but real ![[Cranfield.pdf]]',
			'Cranfield.pdf',
		);
		expect(out.rewritten).toBe(1);
		expect(out.text).toBe(
			'`![[Cranfield.pdf]]` but real ![[Cranfield.pdf.md]]',
		);
	});
	...
	it('fault_malformed_brackets_no_crash', () => {
		expect(() =>
			scanReferences('![[unterminated and [[ stray ]] text'),
		).not.toThrow();
